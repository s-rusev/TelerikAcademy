...
namespace Store.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            IdentityManager = new AuthenticationIdentityManager(new IdentityStore(new StoreDataContext()));
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Success)
                {
                    ...
                    var userRole = await IdentityManager.Roles.FindRoleByNameAsync("User");
        ...
        {
            var context = new StoreDataContext();
            using (context)
            {
                var user = context.Users.FirstOrDefault(x => x.UserName == this.User.Identity.Name);
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await IdentityManager.Authentication.CreateAndSignInExternalUserAsync(AuthenticationManager, new StoreUser() { UserName = model.UserName });
                if (result.Success)
                {
                    return RedirectToLocal(returnUrl);
                }
        ...
        public ActionResult History()
        {
            ...
            if (this.User.Identity.IsAuthenticated)
            {
                ...
                userOrders = db.Orders.Where(p => p.UserId == userId && p.OrderStatus != OrderStatus.Open);
            }
            else
            {
        ...
        public ActionResult Details(int? id)
        {
            ...
            var orders = db.Orders.Find(id).OrderedProducts;
            if (orders == null)
            {
                return HttpNotFound();
            }
            
            return PartialView("_Details", orders);
        }
